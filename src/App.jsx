...
import QRCode from 'react-qr-code' // L'outil pour le QR Code
import './App.css'
...
function App() {
  ...
  const [userRole, setUserRole] = useState('Reception') // Par défaut Réception
  const [loading, setLoading] = useState(false)
  const [searchTerm, setSearchTerm] = useState('')
  
  // --- MODE PATIENT (QR CODE) ---
  ...
  // Structure Paramètres: { nom, valeur, unite, min, max }
  ...
  // URL du Logo (Tu peux mettre le lien de ton image ici)
  ...
  // --- 1. INITIALISATION ---
  useEffect(() => {
    // Vérification : Est-ce qu'on est un patient qui scanne un QR Code ?
    const urlParams = new URLSearchParams(window.location.search);
    const bilanId = urlParams.get('id');

    if (bilanId) {
      // OUI -> On charge juste le dossier pour le patient
      fetchPublicBilan(bilanId)
    } else {
      // NON -> On charge l'app normale
      checkUserSession()
    }
  }, [])

  // Récupérer la session et le ROLE de l'employé
  async function checkUserSession() {
    const { data: { session } } = await supabase.auth.getSession()
    if (session?.user) {
      setUser(session.user)
      // Récupérer le rôle (Poste) de l'employé
      const { data: employe } = await supabase.from('employes').select('poste').eq('id', session.user.id).single()
      if (employe) setUserRole(employe.poste || 'Reception')
      else setUserRole('Biologiste') // Par défaut si pas trouvé
      fetchData()
    }
  ...
  // Pour le patient (Vue Publique)
  async function fetchPublicBilan(id) {
    const { data, error } = await supabase.from('bilans').select('*').eq('id', id).single()
    if (data) {
      setPatientViewBilan(data)
      try {
        if (data.resultat_analyse) setParametres(JSON.parse(data.resultat_analyse))
      } catch { setParametres([]) }
    }
  }
  ...
  // Gestion Paramètres avec Normes
  ...
  // Vérification Normes (Est-ce que c'est Pathologique ?)
  const checkNorme = (val, min, max) => {
    ...
    return (v < mi || v > ma); // Retourne Vrai si hors normes
  }
    ...
    return (
      <div style={{background: 'white', minHeight: '100vh', color: 'black', padding: '20px', fontFamily: 'Arial'}}>
        <div style={{textAlign: 'center', marginBottom: '20px'}}>
           ...
           <p style={{fontSize: '0.8rem', color: 'green'}}>✅ Document Authentifié par QR Code</p>
        </div>
        ...
        <div style={{border: '1px solid #ddd', padding: '15px', borderRadius: '10px', marginBottom: '20px', background: '#f9f9f9'}}>
          ...
          <p>Age: {patientViewBilan.age_patient} ans</p>
        ...
        <table style={{width: '100%', borderCollapse: 'collapse'}}>
          <thead>
            <tr style={{background: '#eee', borderBottom: '2px solid #005b96'}}>
              <th style={{padding: '10px', textAlign: 'left'}}>Paramètre</th>
              <th style={{padding: '10px', textAlign: 'center'}}>Résultat</th>
              <th style={{padding: '10px', textAlign: 'center'}}>Unité</th>
              <th style={{padding: '10px', textAlign: 'center'}}>Normes</th>
            </tr>
          </thead>
          <tbody>
            {parametres.map((p, i) => {
              ...
              return (
                <tr key={i} style={{borderBottom: '1px solid #ddd'}}>
                  ...
                  <td style={{padding: '10px', textAlign: 'center', fontWeight: 'bold', color: isAlert ? 'red' : 'black'}}>
                    {p.valeur} {isAlert && '⚠️'}
                  </td>
                  <td style={{padding: '10px', textAlign: 'center', fontSize: '0.9rem'}}>{p.unite}</td>
                  <td style={{padding: '10px', textAlign: 'center', fontSize: '0.9rem', color: '#666'}}>{p.min} - {p.max}</td>
                </tr>
              )
            })}
          </tbody>
        ...
        <div style={{marginTop: '30px', textAlign: 'center', fontSize: '0.8rem', color: '#999'}}>
          Institut Pasteur d'Algérie - Annexe Dely Ibrahim<br/>
          Ceci est un document numérique officiel.
        </div>
      </div>
    )
  }
  ...
  // --- VUE LOGIN ---
  if (!user) {
    const Background = () => (<div className="bio-background"><ul className="bacteria-list">{[...Array(10)].map((_,i)=><li key={i} className="bacteria"></li>)}</ul></div>)
    return (
      <>
        <Background />
        <div className="login-container">
          <div className="login-card">
            ...
            <form onSubmit={handleLogin}>
              ...
              <button className="btn btn-primary" style={{width:'100%'}}>Connexion Staff</button>
            </form>
          </div>
        </div>
      </>
              ...
              {filteredBilans.map((b) => (
                <tr key={b.id}>
                  ...
                  <td style={{textAlign: 'right'}}>
                    ...
                    {userRole === 'Biologiste' && <button className="btn btn-action btn-danger" style={{marginLeft: '5px'}} onClick={() => {if(confirm('Supprimer ?')) supabase.from('bilans').delete().eq('id',b.id).then(fetchData)}}>🗑️</button>}
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <div className="modal-overlay" onClick={() => setIsModalOpen(false)} style={{position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(5px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100}}>
            <div className="modal" onClick={e => e.stopPropagation()} style={{padding: '30px', borderRadius: '20px', width: '90%', maxWidth: '900px', maxHeight: '90vh', overflowY: 'auto', background: '#1e293b', border: '1px solid rgba(255,255,255,0.1)', color: 'white'}}>
              ...
              <div className="print-header" style={{textAlign: 'center', marginBottom: '20px'}}>
                ...
                <p>Laboratoire d'Analyses Médicales - Annexe Dely Ibrahim</p>
              ...
              <form onSubmit={handleSave} style={{display: 'flex', flexDirection: 'column', gap: '20px'}}>
                ...
                {/* 1. INFOS & QR CODE (Affiche le QR seulement si le dossier existe déjà) */}
                <div style={{display: 'flex', gap: '20px'}}>
                  <div style={{flex: 1, background: 'rgba(255,255,255,0.05)', padding: '15px', borderRadius: '10px'}}>
                    <h3 style={{margin:'0 0 10px 0', fontSize:'0.9rem', color: '#94a3b8', textTransform:'uppercase'}}>Patient</h3>
                  ...
                  {currentBilan.id && (
                    <div style={{background: 'white', padding: '10px', borderRadius: '10px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center'}}>
                      <QRCode value={`${window.location.origin}?id=${currentBilan.id}`} size={100} />
                      <p style={{color: 'black', fontSize: '0.6rem', marginTop: '5px', textAlign: 'center'}}>Scannez pour<br/>résultats</p>
                    </div>
                  )}
                </div>
                ...
                <div style={{background: 'rgba(255,255,255,0.05)', padding: '15px', borderRadius: '10px'}}>
                  <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px'}}>
                    <h3 style={{margin:0, fontSize:'0.9rem', color: '#94a3b8'}}>EXAMENS</h3>
                    <button type="button" className="no-print" onClick={ajouterType} style={{background: 'var(--secondary)', color:'white', border:'none', borderRadius:'4px', fontSize:'0.7rem', padding: '5px'}}>+ Ajouter</button>
                  </div>
                  {typesAnalysesList.map((item, i) => (
                    <div key={i} style={{marginBottom: '5px', display: 'flex', gap: '5px'}}>
                      <select className="form-input" value={item.selection} onChange={e => modifierType(i, 'selection', e.target.value)}>
                        {LISTE_ANALYSES.map(t => <option key={t} value={t}>{t}</option>)}
                      </select>
                      {item.selection === 'Autre' && <input className="form-input" placeholder="Préciser..." value={item.custom} onChange={e => modifierType(i, 'custom', e.target.value)} />}
                      <button type="button" className="no-print" onClick={() => supprimerType(i)} style={{color: '#ef4444', background: 'transparent', border:'none'}}>✕</button>
                    </div>
                  ))}
                </div>

                {/* 3. RESULTATS (Visible uniquement si Biologiste ou pour impression) */}
                {(userRole === 'Biologiste' || currentBilan.statut === 'termine') && (
                  <div style={{background: 'rgba(255,255,255,0.05)', padding: '15px', borderRadius: '10px'}}>
                    <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px'}}>
                      <h3 style={{margin:0, fontSize:'0.9rem', color: '#94a3b8'}}>RESULTATS TECHNIQUES</h3>
                    ...
                    {/* EN-TETES TABLEAU */}
                    <div style={{display: 'flex', gap: '10px', fontSize: '0.7rem', color: '#94a3b8', paddingLeft: '5px', marginBottom: '5px'}}>
                      <div style={{flex: 2}}>PARAMÈTRE</div>
                      <div style={{flex: 1, textAlign: 'center'}}>RÉSULTAT</div>
                      <div style={{flex: 1, textAlign: 'center'}}>UNITÉ</div>
                      <div style={{flex: 1, textAlign: 'center'}}>NORME MIN-MAX</div>
                    ...
                    {parametres.map((param, i) => {
                      ...
                      return (
                        <div key={i} style={{display: 'flex', gap: '10px', marginBottom: '5px', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '5px'}}>
                          <input className="form-input" style={{flex: 2}} placeholder="Nom" value={param.nom} onChange={(e) => modifierParametre(i, 'nom', e.target.value)}/>
                          
                          {/* CHAMP VALEUR : DEVIENT ROUGE SI HORS NORME */}
                          <input 
                            className="form-input" 
                            style={{flex: 1, textAlign: 'center', fontWeight: 'bold', color: isAlert ? '#ef4444' : 'inherit', borderColor: isAlert ? '#ef4444' : '#334155'}} 
                            placeholder="Val" 
                            value={param.valeur} 
                            onChange={(e) => modifierParametre(i, 'valeur', e.target.value)}
                          />
                          
                          <input className="form-input" style={{flex: 1, textAlign: 'center'}} placeholder="Unité" value={param.unite} onChange={(e) => modifierParametre(i, 'unite', e.target.value)}/>
                          
                          <div style={{flex: 1, display: 'flex', gap: '5px'}}>
                            <input className="form-input" style={{textAlign: 'center', fontSize: '0.8rem'}} placeholder="Min" value={param.min} onChange={(e) => modifierParametre(i, 'min', e.target.value)}/>
                            <input className="form-input" style={{textAlign: 'center', fontSize: '0.8rem'}} placeholder="Max" value={param.max} onChange={(e) => modifierParametre(i, 'max', e.target.value)}/>
                          </div>
                          
                          <button type="button" className="no-print" onClick={() => supprimerParametre(i)} style={{background: 'transparent', border: 'none', color: '#ef4444', cursor: 'pointer'}}>✕</button>
                        </div>
                      )
                    })}
                  </div>
                ...
                {/* 4. VALIDATION (Seul le biologiste peut valider) */}
                <div style={{background: 'rgba(255,255,255,0.05)', padding: '15px', borderRadius: '10px'}}>
                  <label>Statut & Validation</label>
                  <select className="form-input" disabled={userRole === 'Reception'} value={currentBilan.statut} onChange={e => setCurrentBilan({...currentBilan, statut: e.target.value})}>
                    <option value="en_attente">En Attente</option>
                    <option value="en_cours">En Cours</option>
                    <option value="termine">Terminé & Validé (Signature Biologiste)</option>
                  </select>
                  {userRole === 'Reception' && <p style={{fontSize: '0.7rem', color: '#fb923c', marginTop: '5px'}}>⚠️ Réservé au Biologiste</p>}
                </div>
              ...
              <div className="print-footer">
                <div style={{display: 'flex', justifyContent: 'space-between', marginTop: '50px'}}>
                   <div><p>Fait à Alger, le {new Date().toLocaleDateString()}</p></div>
                   <div style={{textAlign: 'center'}}><p>Le Biologiste</p><br/><br/><br/>Signé</div>
                </div>
                <p style={{textAlign: 'center', marginTop: '20px', fontSize: '0.7rem'}}>Institut Pasteur d'Algérie - Généré par PasteurLab System</p>
              </div>
